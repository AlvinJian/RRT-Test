...
{	//return 1 if no collision occurrs
	...
	while( !nearSide(tmp.x, tmp.y, p2->x, p2->y, 0.01) )
	{
		...
		{
            if(nearSide(tmp.x, tmp.y, obs[i].first, obs[i].second, 0.6))
				return 0;
		}
...
bool t_connect(vector<node*>& total, node* Xrand, vector<pair<double, double> >& obs)
{
	...
	{
		pair<node*, double> p_edge; p_edge.first = total[i];
		p_edge.second = d2edgeCalc(total[i], total[i]->parent, Xrand);
...
{
    //this part has a serious problem
    while(1)
    {
    ...
    for(unsigned int i = 0; i < path.size() - 1; ++i)
    {
        ...
        if(dist < step)
        {
            ...
            if(!add)
            {
                ...
                rslt_path[rslt_path.size()-1]->parent=path[path.size()-1];
